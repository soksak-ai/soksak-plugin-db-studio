...
export function registerCommands(ctx: PluginContext, store: ErdStore): void {
  ...
  add('probe-clickpath', 'Diagnose the open dialog: modal opacity/background, overlay & control pointer-events, input reachability, autofocus target (E2E overlay/clickability assertion; settles enter animations for deterministic capture)', { ko: '클릭 경로 진단 오버레이 포인터이벤트 다이얼로그 모달' }, (d) => d.dialogOpen ? `모달 표시 ${d.contentPaint?.backgroundColor && d.contentPaint.backgroundColor !== 'rgba(0, 0, 0, 0)' ? '정상(불투명)' : '결함(투명)'}` : '열린 다이얼로그 없음', () => {
    if (typeof document === 'undefined') return { ok: true, dialogOpen: false, reason: 'no document (headless)' };
    // Shadow DOM 관통 히트테스트 — 앱은 shadow host 로 마운트되므로 document.elementFromPoint 는
    // host 만 반환한다. shadowRoot 를 따라 내려가 실제 최상단 요소를 찾는다(실제 마우스와 동일 규칙).
    ...
    const pe = (el: Element | null) => (el ? getComputedStyle(el).pointerEvents : null);
    const roots: (Document | ShadowRoot)[] = [document];
    // 다이얼로그는 shadow host 로 포털되므로 shadow 안에서도 찾는다.
    ...
    // 실제 마우스가 el 중심을 눌렀을 때 el(또는 그 자식)에 닿는가 — pointer-events:none 이면
    // 히트가 el 을 관통해 뒤 요소로 빠지므로 도달 실패.
    ...
    // Table Name 입력 = 다이얼로그 내 첫 input. Create 버튼 = 푸터의 마지막 버튼.
    const input = content?.querySelector('input') ?? null;
    const primary = content?.querySelector('[data-slot="dialog-footer"] button:last-of-type') as HTMLButtonElement | null;
    // autoFocus 대상(포털이 shadow 안이면 activeElement 는 shadow 안을 가리킨다).
    const findActive = (): Element | null => {
      let ae: Element | null = document.activeElement;
      while (ae?.shadowRoot?.activeElement) ae = ae.shadowRoot.activeElement;
      return ae;
    };
    // 캡처 보정: 진입 애니메이션(animate-in fade-in)을 끝 상태로 강제 완료한다. 배경 웹뷰는
    // 애니메이션 타임라인이 멈춰 opacity 0 에 갇히므로(렌더가 아니라 타임라인 정지), 눈검증
    // 스냅샷이 최종(불투명) 모달을 보게 한다. finish() 는 타임라인 정지와 무관하게 seek 한다.
    let settledOpacity: string | null = null;
    if (content) {
      try {
        for (const el of [content, overlay]) {
          (el as Element & { getAnimations?: (o?: { subtree?: boolean }) => Animation[] })
    ...
    const ae = findActive();
    // 모달 카드가 불투명하게 칠해지는가 — bg-background 가 shadow 포털에서 var(--background)
    // 미해결로 투명해지면 어두운 오버레이가 모달을 관통해 비쳐 "오버레이 아래 모달"처럼 보인다.
    const paint = (el: Element | null, label: string) => {
      if (!el) return { [label]: null } as Record<string, unknown>;
      ...
      return {
        [label]: {
          ...
          opacity: cs.opacity,          // animate-in 이 rAF 정지로 0 에 멈췄는지(캡처 아티팩트 판별)
    ...
    return {
      ok: true,
      dialogOpen: !!content,
      bodyPointerEvents: pe(document.body),
      overlayPointerEvents: pe(overlay),
      contentPointerEvents: pe(content),
      ...
      activeElement: ae ? { tag: ae.tagName.toLowerCase(), type: (ae as HTMLInputElement).type ?? null, isTableNameInput: ae === input } : null,
      ...
      settledOpacity,                 // 진입 애니메이션 완료 후 모달 opacity(1 이면 완전 표시)
      // 모달 카드/오버레이 페인트 진실 — 모달이 불투명 배경을 갖는가(투명이면 오버레이가 비친다).
      ...paint(content, 'contentPaint'),
      ...paint(overlay, 'overlayPaint'),
      portalRootBgVar: (() => {
        // 포털 컨테이너(shadow host)에서 --background 가 정의돼 있는가.
        const c = content?.parentElement ?? null;
        return c ? (getComputedStyle(c).getPropertyValue('--background').trim() || null) : null;
      })(),
    };
  });
